...
class UserController extends Controller
{
    ...
    {
        $user = Auth::user();
        return view('front.ShuwaikhCoffe.user-profile.profile', compact('user'));
    }

    public function EditeProFile()
    {
        $user = Auth::user();
        return view('front.ShuwaikhCoffe.user-profile.edite-profile', compact('user'));
    }
    public function profileupdate(Request $request)
    {
        ...
        $request->validate([
            ...
            'number' => 'nullable|regex:/^([0-9\s\-\+\(\)]*)$/|min:10|max:15',
            'photo' => [
        ...
        $input = $request->only(['address', 'photo', 'old_password', 'new_password', 'number']);
        ...
        $data->update(array_filter($input, function ($value) {
            return $value !== '' || $value === null;
        }));

        $previousUrl = session('previous_url', url('/'));
        session()->forget('previous_url');
        return redirect($previousUrl)->with('success', 'Profile updated successfully.');
    }
    ...
    public function shippingupdate(Request $request)
    {
        ...
        $user->update($request->all() + ['shipping_country_code' => 965]);

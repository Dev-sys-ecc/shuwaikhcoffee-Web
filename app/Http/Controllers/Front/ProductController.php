...
class ProductController extends Controller
{
    ...
    public function ProductPageDetails($slug,$id)
    {
        if (session()->has('lang')) {
            $currentLang = Language::where('code', session()->get('lang'))->first();
        } else {
        ...
        if ($data['product']) {
            return view('front.ShuwaikhCoffe.products.details', $data);
        }
    ...
    public function offers()
    {
        ...
        $data['offers'] = Offer::where('language_id', $currentLang->id)
            ->where('status', 1)
            ->where('start_date', '<=', now())
            ->where('end_date', '>=', now())
            ->orderBy('id', 'desc')
            ->get();
        $data['blogs'] = Blog::where('language_id', $currentLang->id)->orderBy('id', 'desc')->take(6)->get();
        return view('front.ShuwaikhCoffe.products.offers', $data);
    }
    public function offerPageDetails($id)
    {
        if (session()->has('lang')) {
            $currentLang = Language::where('code', session()->get('lang'))->first();
        } else {
        ...
        $data['offer'] = Offer::where('id', $id)->where('language_id', $currentLang->id)->first();

        if ($data['offer']) {
            return view('front.ShuwaikhCoffe.products.offer_detail', $data);
        }
    }
    ...
    public function items(Request $request)
    {
        ...
        $data['products'] = Product::where('status', 1)->where('language_id', $currentLang->id)->get(['id', 'title', 'slug', 'category_id' ,'feature_image', 'current_price', 'previous_price', 'addons', 'variations']);
		
        $data['categories'] = Pcategory::where('status', 1)->where('language_id', $currentLang->id)->get(['id' , 'slug' , 'name' , 'image']);
        ...
        return view('front.ShuwaikhCoffe.products.products', $data);
    }
    ...
    public function cart()
    {
        ...
        return view('front.ShuwaikhCoffe.products.cart', compact('cart' , 'lang_id'));
    }
    ...
    {
        return view('front.ShuwaikhCoffe.products.wishlist');
    }
    public function wishlistStore(Request $request)
    {
        ...
        {
            return response()->json(['status' => false, 'message' => "Item Is Aleady In Wishlist"]);
            ...
            if($result)
            {
                return response()->json(['status' => true, 'message' => "Item Added  In Wishlist" , 'wishlist_count' => Cart::instance('wishlist')->count()]);
            }
        }
    }
    public function wishlistDelete(Request $request)
    {
        ...
        return response()->json(['status' => true, 'message' => "Item has been removed to a wishlist" , 'wishlist_count' => Cart::instance('wishlist')->count()]);
...
//    {
//        if ($request->type != 'guest' && !Auth::guard('web')->check()) {
//            Session::put('link', route('front.checkout'));
//            return redirect(route('user.login', ['redirected' => 'checkout']));
//        }
//
//        if (auth('web')->check() && (auth('web')->user()->address == null || auth('web')->user()->state == null)) {
//            Session::flash('error', 'Enter the address and state ');
//            return redirect()->route('edite-user-profile');
//        }
//
//        if (session()->has('lang')) {
//            $currentLang = Language::where('code', session()->get('lang'))->first();
...
//
//        $data['ccodes'] = [["code" => "+965", "name" => "Kuwait"]];
//
//        return view('front.ShuwaikhCoffe.products.checkout1', $data);
//    }
    ...
    public function checkout(Request $request)
    {
        ...
        return view('front.ShuwaikhCoffe.products.checkout', $data);
    }

               ...
               <div class="card-body">
                   @if(count($user->offers))
                       @foreach ($user->offers as $offer)
                           <p>{{ $offer->title }}</p>
                           <a href="{{ route('admin.offer.unsubscribe', ['offer' => $offer->id, 'user' => $user->id]) }}" class="btn btn-sm btn-danger">الغاء الاشتراك</a>
                       @endforeach
                   @else
                       <p class="text-danger">المستخدم غير مشترك في اي عروض</p>
                   @endif
               </div>
